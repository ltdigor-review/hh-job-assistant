...
- Analyze extension logs from a user-provided `.debug` file with `npm run inspect:logs -- --file <path>`, or inspect local Chrome/Chromium profile storage with `npm run inspect:logs -- --storage-dir <Local Extension Settings path>`. Current history uses `agentDebugRunIndex`, `agentDebugActiveRunId`, and `agentDebugRun:<runId>`; `runState` and `runResults` remain separate runtime evidence.
