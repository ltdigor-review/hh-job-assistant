...
function validateHumanShortCoverLetter(value) {
  ...
  if (/(?:уважаем|меня\s+привлекла|инновац|эффективност|масштабн|соответству(?:ет|ю)\s+требованиям|требования\s+вакансии|проявлял(?:а)?\s+интерес|готов(?:а)?\s+(?:обсудить|применять)\b|релевантн(?:ый|ого|ом)\s+опыт|чем\s+могу\s+быть\s+полезен|буду\s+рад)/i.test(text)) {
    return 'cliche';
  }
  return '';
}
