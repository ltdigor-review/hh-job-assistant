...
- Resume refresh flow.
- Chat assistant flow and reports.
...
| Done | Chat assistant unit/DOM flows | Covered by `tests/content-chat-assist.test.mjs`; draft, auto-send, unread-only, external-contact report, markdown stripping, and bad-reply skip |
...
| Done | Options load current settings | Options showed model, resume URL, expected salary, delays, chat settings, masked Groq key |
...
| Done | Live 20 auto-applications | Extension submitted 19 applications in run `2026-06-08T11:25:02.348Z`; extra extension run from `2026-06-08T11:28:43.660Z` submitted 1 more; total live extension applications: 20 |
| Done | Auto-apply with employer questions | Live run included `applied_test_assisted` vacancies `133965069`, `133938848`, and extra `129656991` |
...
| Done | Chat assistant draft mode | `tests/content-chat-assist.test.mjs` covers draft-only fill without send, unread-only, reports, external-contact skip, generated-text sanitization and validation |
| Done | Clear reports/debug log actions | `tests/extension-build.test.mjs` covers `CLEAR_CHAT_REPORTS` and `CLEAR_AGENT_DEBUG_LOG` storage behavior |
...
- Refresh/save resume.
- Run chat assistant in draft-only mode.

Original live side-effect scope:

1. Send resume/profile, vacancy, question, and chat text to Groq with saved Groq API key.
...
4. Run chat assistant in draft-only mode, which may fill message drafts but should not send chat messages.
...
- Current local extension storage shows latest live preview state: `found=50`, `processed=50`, `skipped=8`, `errors=0`, `state=dry_run_complete`, updated `2026-06-08T10:40:49.640Z`.
